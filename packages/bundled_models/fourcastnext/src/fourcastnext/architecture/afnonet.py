...
class AFNO2D(nn.Module):
    def __init__(
        ...
        self.b1 = nn.Parameter(
            torch.empty(2, self.num_blocks, self.block_size * self.hidden_size_factor)
        )
        trunc_normal_(self.b1, std=0.02)
        ...
        trunc_normal_(self.b2, std=0.02)
...
class AFNONet(nn.Module):
    def __init__(
        ...
        self.flow_head = nn.Linear(
            embed_dim,
            self.out_chans * self.patch_size[0] * self.patch_size[1] * 2,
            bias=False,
        )
